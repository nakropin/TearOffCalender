    ...
    render() {
        ...
        function getCoordinates(e) { /* get mouseCoords at start */
            ...
            if (mouseX > 0) {
                bezierPoints = [{x: centerX, y: centerY}, {x: mouseX + centerX, y: mouseY}, {
                    x: 0,
                    y: targetY
                }, {x: targetX + Math.random() * 100 - 50, y: targetY}];
            } else {
        ...
        function animatePage() {
            ...
            if (renderPageCallCounter < randomFiles.length) {
                ...
                const rotationXfactor = 1.1;
                const rotationZfactor = 0.8;
                const animateOnce = () => {
                    let position = getBezierPosition(bezier, progress);
                    let rotationAngle = Math.atan2(position.x, position.y) * 180 / Math.PI + random;
                    curPage.style.transform = 'translate(' + position.x + 'px, ' + position.y + 'px) rotateX(' + rotationAngle * progress * rotationXfactor + 'deg) rotateZ(' + -rotationAngle * progress * rotationZfactor + 'deg)';
                    if (progress < 1) {
                        progress += 0.01;
